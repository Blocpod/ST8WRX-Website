(() => {
  ...
  // ---------- GLOBE: compact GSHHS coastline topology + corrected east-positive orthographic projection ----------
  (()=>{
    const canvas=document.getElementById('worldGlobe'); if(!canvas) return; const ctx=canvas.getContext('2d'); if(!ctx) return;
    const encoded=window.__ST8_GSHHS||'';
    if(!encoded){ console.warn('ST8WRX GSHHS topology unavailable'); return; }
    let raw; try{ raw=atob(encoded); }catch(err){ console.warn('ST8WRX GSHHS topology invalid',err); return; }
    const bytes=new Uint8Array(raw.length); for(let i=0;i<raw.length;i++) bytes[i]=raw.charCodeAt(i);
    const dv=new DataView(bytes.buffer); let off=0;
    if(dv.byteLength<4) return;
    const segCount=dv.getUint16(off,true); off+=2; const coasts=[];
    for(let s=0;s<segCount;s++){
      if(off+2>dv.byteLength) return; const len=dv.getUint16(off,true); off+=2; const seg=[];
      for(let i=0;i<len;i++){ if(off+4>dv.byteLength) return; seg.push([dv.getInt16(off,true)/100,dv.getInt16(off+2,true)/100]); off+=4; }
      if(seg.length>1) coasts.push(seg);
    }
    if(off+2>dv.byteLength) return; const landCount=dv.getUint16(off,true); off+=2; const land=[];
    for(let i=0;i<landCount;i++){ if(off+4>dv.byteLength) break; land.push([dv.getInt16(off,true)/100,dv.getInt16(off+2,true)/100]); off+=4; }
    ...
    function project(latD,lonD,lon0D,lat0D){const p=rad(latD),l=rad(lonD),l0=rad(lon0D),p0=rad(lat0D),dl=l-l0,cp=Math.cos(p),sp=Math.sin(p),c0=Math.cos(p0),s0=Math.sin(p0);const X=cp*Math.sin(dl),Y=c0*sp-s0*cp*Math.cos(dl),Z=s0*sp+c0*cp*Math.cos(dl);return{x:cx+R*X,y:cy-R*Y,z:Z}}
    ...
    function coast(seg,lon0,lat0){ctx.beginPath();let pen=false;for(const [lon,lat] of seg){const p=project(lat,lon,lon0,lat0);if(p.z>.005){pen?ctx.lineTo(p.x,p.y):ctx.moveTo(p.x,p.y);pen=true}else pen=false}ctx.strokeStyle='rgba(255,174,58,.62)';ctx.lineWidth=.76;ctx.stroke()}
    function grid(lon0,lat0){ctx.save();ctx.lineWidth=.5;for(let lat=-60;lat<=60;lat+=20){ctx.beginPath();let pen=false;for(let lon=-180;lon<=180;lon+=4){const p=project(lat,lon,lon0,lat0);if(p.z>0){pen?ctx.lineTo(p.x,p.y):ctx.moveTo(p.x,p.y);pen=true}else pen=false}ctx.strokeStyle='rgba(240,211,158,.075)';ctx.stroke()}for(let lon=-180;lon<180;lon+=20){ctx.beginPath();let pen=false;for(let lat=-88;lat<=88;lat+=3){const p=project(lat,lon,lon0,lat0);if(p.z>0){pen?ctx.lineTo(p.x,p.y):ctx.moveTo(p.x,p.y);pen=true}else pen=false}ctx.strokeStyle='rgba(240,211,158,.055)';ctx.stroke()}ctx.restore()}
    function route(a,b,lon0,lat0,time,idx){const pts=[];for(let i=0;i<=48;i++){const t=i/48,[lat,lon]=slerp(a,b,t),p=project(lat,lon,lon0,lat0),lift=1+.075*Math.sin(Math.PI*t);pts.push({...p,x:cx+(p.x-cx)*lift,y:cy+(p.y-cy)*lift})}ctx.beginPath();let pen=false;for(const p of pts){if(p.z>-.02){pen?ctx.lineTo(p.x,p.y):ctx.moveTo(p.x,p.y);pen=true}else pen=false}ctx.strokeStyle='rgba(255,185,73,.25)';ctx.lineWidth=.78;ctx.stroke();const t=reduce?.55:((time*.000032+idx*.127)%1),p=pts[Math.min(47,Math.floor(t*48))];if(p&&p.z>0){ctx.save();ctx.globalCompositeOperation='lighter';ctx.shadowColor='rgba(255,151,14,.92)';ctx.shadowBlur=14;ctx.beginPath();ctx.arc(p.x,p.y,1.9,0,TAU);ctx.fillStyle='rgba(255,216,137,.98)';ctx.fill();ctx.restore()}}
    function draw(time){
      ctx.setTransform(dpr,0,0,dpr,0,0);ctx.fillStyle='#050505';ctx.fillRect(0,0,w,h);
      const lon0=-52+(reduce?0:(time-start)*.00062)+hoverX*16,lat0=18-hoverY*8;
      const body=ctx.createRadialGradient(cx-R*.34,cy-R*.30,R*.04,cx,cy,R*1.05);body.addColorStop(0,'#563007');body.addColorStop(.25,'#2e1907');body.addColorStop(.62,'#100b08');body.addColorStop(1,'#050505');ctx.fillStyle=body;ctx.beginPath();ctx.arc(cx,cy,R,0,TAU);ctx.fill();
      ctx.save();ctx.beginPath();ctx.arc(cx,cy,R-.5,0,TAU);ctx.clip();ctx.globalCompositeOperation='lighter';
      for(const [lon,lat] of land){const p=project(lat,lon,lon0,lat0);if(p.z<=.015)continue;const lum=.10+.34*Math.pow(p.z,.8),rr=.46+.72*p.z;ctx.beginPath();ctx.arc(p.x,p.y,rr,0,TAU);ctx.fillStyle=`rgba(255,${Math.round(126+80*p.z)},${Math.round(16+50*p.z)},${lum})`;ctx.fill()}
      ctx.globalCompositeOperation='source-over';grid(lon0,lat0);coasts.forEach(s=>coast(s,lon0,lat0));links.forEach((l,i)=>route(hubs[l[0]],hubs[l[1]],lon0,lat0,time,i));
      hubs.forEach((n,i)=>{const p=project(n[1],n[2],lon0,lat0);if(p.z<=0)return;const pulse=reduce?1:.84+.16*Math.sin(time*.0017+i);ctx.save();ctx.globalCompositeOperation='lighter';ctx.shadowColor='rgba(255,144,0,.9)';ctx.shadowBlur=12;ctx.beginPath();ctx.arc(p.x,p.y,(1.6+1.1*p.z)*pulse,0,TAU);ctx.fillStyle='rgba(255,194,91,.96)';ctx.fill();ctx.restore()});ctx.restore();
      ctx.save();ctx.shadowColor='rgba(255,145,18,.26)';ctx.shadowBlur=22;ctx.strokeStyle='rgba(255,172,50,.24)';ctx.lineWidth=1;ctx.beginPath();ctx.arc(cx,cy,R,0,TAU);ctx.stroke();ctx.restore();
    }
    function loop(t){if(t-last>38){last=t;draw(t)}if(!reduce&&visible&&!document.hidden)raf=requestAnimationFrame(loop);else raf=0}
    resize();draw(performance.now());if('ResizeObserver'in window)new ResizeObserver(()=>{resize();draw(performance.now())}).observe(canvas);if('IntersectionObserver'in window)new IntersectionObserver(e=>{visible=!!e[0]?.isIntersecting;if(visible&&!raf&&!reduce)raf=requestAnimationFrame(loop)},{rootMargin:'160px'}).observe(canvas);const host=canvas.parentElement;if(host&&matchMedia('(pointer:fine)').matches){host.addEventListener('pointermove',e=>{const r=host.getBoundingClientRect();hoverX=(e.clientX-r.left)/r.width-.5;hoverY=(e.clientY-r.top)/r.height-.5},{passive:true});host.addEventListener('pointerleave',()=>{hoverX*=.2;hoverY*=.2},{passive:true})}if(!reduce)raf=requestAnimationFrame(loop)
  })();
})();
